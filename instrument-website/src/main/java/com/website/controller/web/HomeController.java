...
//		model.setTotalPage((int) Math.ceil((double) model.getTotalItem()/model.getMaxPageItem()));
		...
		request.setAttribute("cartCount",
				CountCart.countCart((Map<Long, Cart>) SessionUtil.getInstance().getValue(request, "cart")));
		...
		if (action != null && action.equals("login")) {
			...
			if (model != null) {
				if (model.getStatus() == 1) {
					...
					response.sendRedirect(request.getContextPath() + "/login?action=login&message=disable&alert=danger");
				}
			} else {
